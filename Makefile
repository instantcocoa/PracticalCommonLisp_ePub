...
GENERALOPTS = -f html+smart --standalone --table-of-contents -V title:"Practical Common Lisp" -V author:"Peter Seibel" -V date:"June 29, 2012 - Build ${BUILD}"
...
create_folder:
	if [ ! -d "${DIR}" ]; then \
		mkdir ${DIR}; \
		cp -r html/figures ${DIR}/; \
		cp -r html/screenshots ${DIR}/; \
		cp html/*.html html/*.jpg html/*.png html/*.css ${DIR}/ 2>/dev/null || true; \
	fi
